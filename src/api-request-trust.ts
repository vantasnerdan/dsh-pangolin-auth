...
function isTrustedAuthorityAndOrigin(
...
/**
 * Decide whether one frontend-index request uses an owned Host and matching Origin.
 * Cross-site top-level navigation remains valid because an SSO callback can arrive
 * from a different site before Pangolin injects the authenticated identity.
 * @param request - Node HTTP request facts.
 * @param trustedHosts - non-loopback authorities this deployment serves.
 * @returns true when the Host is ours and an attached Origin matches it.
 */
export function isTrustedIndexRequest(
  request: ConnectionTrustRequest,
  trustedHosts: readonly string[],
): boolean {
  return isTrustedAuthorityAndOrigin(request, trustedHosts)
}

/**
 * Decide whether one /api or WebSocket request may reach the RPC bridge.
 * @param request - Node HTTP or Fetch request facts.
 * @param trustedHosts - non-loopback authorities this deployment serves.
 * @returns true when Host and Origin are trusted and the browser did not mark the request cross-site.
 */
export function isTrustedApiRequest(
  ...
  return header(request.headers, 'sec-fetch-site') !== 'cross-site'
    && isTrustedAuthorityAndOrigin(request, trustedHosts)
}
