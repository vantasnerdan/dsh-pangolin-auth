...
import type { PangolinAuth } from './pangolin-auth.ts'
import type {
...
export class HostConnectionService extends Service implements HostConnectionHandle {
   ...
   * @param browserAuth - Pangolin forwarded-identity authenticator.
   */
  constructor(
    ctx: Context,
    private readonly trustedHosts: readonly string[],
    private readonly browserAuth: PangolinAuth,
  ) {
  ...
  /** Authenticate an index request after applying its Host/Origin trust checks. */
  authorizeIndex(request: ConnectionIndexRequest, response: ConnectionIndexResponse): boolean {
    if (!isTrustedIndexRequest(request, this.trustedHosts)) {
  ...
  /** Return the configured Pangolin browser-facing URL. */
